...
public class DiaryController {
    ...
    ResponseEntity<List<Diary>> readDiary(
            @RequestParam
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
            @Parameter(description = "조회할 날짜", example = "2025-02-26") LocalDate date) {
...
//        }
    ...
    List<Diary> readDiaries(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                            @Parameter(name = "조회할 기간의 첫 번째 날", example = "yyyy-MM-dd") LocalDate startDate,

                            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                            @Parameter(name = "조회할 기간의 마지막 날", example = "2023-04-01") LocalDate endDate) {
